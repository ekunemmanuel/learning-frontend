...
const verifyType = computed<'email_verification' | 'phone_verification' | 'password_reset'>(() => {
  return (
    ...
    (route.query.type as 'email_verification' | 'phone_verification' | 'password_reset' | undefined) ||
    'email_verification'
  )
})
...
const isPinValid = computed(() => pinValue.value.join('').length === 4)
...
async function handleVerify() {
  ...
  if (code.length < 4) {
    toast.add({
      ...
      description: 'Please fill in all 4 digits of the verification code.',
      color: 'error',
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      ...
      description: msg || 'Code verified successfully!',
      color: 'success',
    })
    ...
    toast.add({
      title: 'Verification Failed',
      description: (err as Error).message || 'Verification failed. Please check the code and try again.',
      color: 'error',
    })
  } finally {
...
async function handleResend() {
  ...
  if (!identifier.value) {
    toast.add({
      title: 'Resend Failed',
      description: 'Missing recipient information for resending code.',
      color: 'error',
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      title: 'Code Sent',
      description: msg || 'A new 4-digit code has been sent to your email.',
      color: 'success',
    })
    startTimer(60)
  } catch (err: unknown) {
    toast.add({
      title: 'Resend Failed',
      description: (err as Error).message || 'Failed to resend verification code.',
      color: 'error',
    })
  } finally {
  ...
  <UCard class="w-full max-w-md shadow-xl border border-default">
    <template #header>
      <div class="text-center space-y-2">
        ...
        <div
          class="inline-block px-3 py-1 bg-primary/10 rounded-full border border-primary/20"
        >
    ...
    <div class="space-y-6 text-center py-2">
      <UFormField
        ...
        required
        help="Enter the 4-digit code sent to your inbox"
    ...
    <template #footer>
      <div class="space-y-4 pt-1">
        ...
        <div
          class="flex items-center justify-between text-xs border-t border-default pt-3"
        >
          <RouterLink
            to="/login"
          ...
          <RouterLink
            ...
            class="text-muted hover:text-highlighted font-medium"
          >
